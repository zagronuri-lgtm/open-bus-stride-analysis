...
| תכנון | `fetch_planned` - gtfs_rides פר יום, מפתח (מפעיל, line_ref) | `fetch_planned_agg` + `fetch_rides_for_routes` פר-מסלול במקביל | לא מושך; קורא JSON יומי |
...
| לוח-החרגות | `src/exclusions_calendar.py` על `data/reference/exclusions_2026/` (7 קבצים, טיפול drop/segment/keep + דריסות פר-סניף) | `load_holidays` על קובץ-חגים משלו (`HOLIDAYS_FILE`) | `classify_days` משלו (סוג-יום מתאריך) |
...
2. **סוג-יום ולוח-חגים** - `exclusions_calendar.py` (מקור-אמת מתועד, כולל דריסות פר-סניף) מול `load_holidays`/`day_type` של daily_nonperf ו-`classify_days` של nonperf_cumulative. **שני לוחות = סיכון:** יום שמוחרג במקום אחד ונספר במקום אחר.
3. **אורכי-מסלול מ-GTFS** - `load_gtfs_lengths` (כאן) מול `load_lengths` (מצטבר); שיטות שונות (haversine על shapes מול shape_dist_traveled).
...
| אורכי-מסלול מ-GTFS | להכריע: shape_dist_traveled (מצטבר) מדויק יותר מ-haversine (כאן) - לאמץ את של המצטבר | weekly_report |
