...
# --------------------------------------------------------------------------
# hotfix — הנתיב האשכולי של הקנס הקבוע נוטרל
# (נספח כ"ו: הקנס הקבוע מוגדר ברמת קו-יום, לא אשכול-יום; מימוש פר-משטר-מכרז
#  ממתין לאישור. עד אז אסור שהנתיב האשכולי הישן ייצר קנס.)
# --------------------------------------------------------------------------
def test_cluster_level_fixed_penalty_is_neutralized():
    """אשכול-יום מעל 4.5% מסומן over_threshold אך אינו מייצר קנס קבוע."""
    d = dt.date(2026, 8, 12)
    recs = [W.LineDayRecord(date=d, op=15, line="L1", cluster="אשכול א",
                            planned=100, executed=50, nonexec=50)]
    rows = W.compute_penalties([_day(d)], recs)
    assert rows, "צפויה שורת קנס אחת לאשכול-יום"
    assert rows[0].over_threshold is True, "חציית 4.5% עדיין מסומנת להדגשה"
    assert all(p.fixed_penalty == 0 for p in rows), \
        "הנתיב האשכולי של הקנס הקבוע חייב להישאר מנוטרל"
